...
(() => {
  const PROGRAM_VERSION = "2026-08-27-v7.2.4-max8-except-abs-legs";
  const versionLabel = document.querySelector(".version");
  if (versionLabel) versionLabel.textContent = "v7.2.4";
  if (typeof state === "undefined" || !state) return;
  if (state.v723ProgramVersion === PROGRAM_VERSION) return;

  const raw = storage.getItem(LS_STATE);
  if (raw) storage.setItem(`${LS_STATE}_v724_yedek_${Date.now()}`, raw);
  ...
  const updates = {
    ...
    v72_cable_crunch: {repMin:12, repMax:12, weight:20},
    ...
    v72_hanging_knee_raise: {repMin:12, repMax:12, weight:0}
  };
